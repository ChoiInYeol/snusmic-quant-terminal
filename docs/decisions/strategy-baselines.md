...
1. **Oracle / investment-god upper bound** — an impossible future-informed
   trader that knows the post-publication path, buys at the best later low, and
   sells at the best later high. This is a ceiling and should not be beaten by a
   realistic strategy under the same price universe.
...
| Field | Meaning |
| --- | --- |
| `oracle_entry_price` | Future-known best buy price after publication. |
| `oracle_exit_price` | Best sell price after the oracle entry. |
| `oracle_return` | `oracle_exit_price / oracle_entry_price - 1`. |
...
should normally hold because the oracle is allowed to choose a better entry and
exit with future information. If this invariant appears violated, inspect data
