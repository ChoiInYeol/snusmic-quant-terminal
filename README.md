...
3. **계좌 시나리오**
   - v3 walk-forward 전략은 초기 투자금 1,000만원과 매월 100만원 추가 납입을 고정합니다.
   - 리밸런싱 날에는 선택된 보유군 안에서 비중을 다시 맞춰 현금이 남지 않도록 투자합니다.
   - 스믹 추종자는 `1/N` 비중만 사용하고, 목표가 도달 시 반드시 매도합니다. 목표가에 도달하지 않은 하락 구간은 계속 들고 가며 다음 달 저축액을 같은 원칙으로 투입합니다.
   - 예언자(`oracle`)는 같은 구간의 미래 가격 경로를 알고 있다고 가정해, long-only 투자에서 가장 효율적인 종목/비중을 선택하는 불가능한 상한선입니다.
4. **현실 모델 전략**
